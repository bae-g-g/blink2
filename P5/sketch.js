...
function setup() {
  createCanvas(1255, 600); // 600,600 사이즈의 캔버스를 생성합니다.
...
if(hands.length == 1){ // 손이 화면에 1개만 인식 될 때 모션을 분석합니다.
    ...
    if(hands[0].keypoints[2].x > hands[0].keypoints[17].x ){ //손바닥 -- 오른손일 때 엄지가 소지보다 왼쪽에 있는경우는 손바닥으로 판단
        ...
        if(pinger_num == 0){ // 펴진 손가락이 0개라면 emergency모드
          ...
          if(hands[0].keypoints[10].x < hands[0].keypoints[12].x ){ // 중지의  손가락끝끝이 마디보다 왼쪽에 있으면 중지가 펴져있다고 판단합니다.
              pinger_num++; // 펴진 손가락 갯수 + 1
          }
          if( hands[0].keypoints[14].x < hands[0].keypoints[16].x ){ // 약지의  손가락끝끝이 마디보다 왼쪽에 있으면 약지가 펴져있다고 판단합니다.
              pinger_num++; // 펴진 손가락 갯수 + 1
          }
          if( hands[0].keypoints[18].x < hands[0].keypoints[20].x){ // 소지의  손가락끝끝이 마디보다 왼쪽에 있으면 소지가 펴져있다고 판단합니다.
...
function motion_guide(){
  ...
  push() //이후 사용될 textSize,tine등 설정옵션을 push합니다. 
  ...
  pop() // textSize,tine등 설정옵션을 pop합니다. 
